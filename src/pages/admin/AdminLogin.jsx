...
export default function AdminLogin({ signIn }) {
  ...
  const handleLogin = async (e) => {
    ...
    if (!passwordValue) return setError("Digite a senha.");

    setLoading(true);
    try {
      await signIn(raw, passwordValue, ROLES.ADMIN);
    ...
    <div className="flex-1 p-6 flex flex-col gap-5">
      ...
      <form onSubmit={handleLogin} className="flex flex-col gap-4">
        <div>
          ...
          <input id="cpf" name="cpf" autoComplete="username" type="text" inputMode="numeric" placeholder="000.000.000-00" value={cpf} onChange={(e) => setCpf(applyMask(e.target.value))} className="input-field" />
        </div>
        <div>
          <label className="block text-sm font-semibold mb-1.5">Senha</label>
          <div className="relative">
            <input id="password" name="password" autoComplete="current-password" type={showPw ? "text" : "password"} placeholder="Senha" value={password} onChange={(e) => setPassword(e.target.value)} className="input-field pr-12" />
