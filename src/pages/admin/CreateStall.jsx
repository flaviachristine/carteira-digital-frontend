...
import { createStall } from "../../services/adminService";

export default function CreateStall({ adminLoggedIn, loading: booting, signOut }) {
  ...
  const validateForm = () => {
    setError("");
    if (!stallName.trim())
      return setError("Digite o nome da barraca.");
  ...
  const handleCreate = async () => {
    ...
    try {
      const raw = onlyDigits(cpf);
      await createStall(stallName, raw, password);
      setSuccess(true);
    } catch (err) {
      setError(err.friendlyMessage || "Não foi possível criar a barraca. Tente novamente.");
      setLoading(false);
    }
  };
      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center px-6">
          <SuccessScreen
            ...
            subtitle={`${stallName} foi cadastrada com sucesso.`}
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <PageHeader title="Criar Barraca" onBack={() => navigate("/admin/dashboard")} />
      <div className="flex-1 p-6 flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-4">
          <div>
            <label className="block text-sm font-semibold mb-1.5">Nome da barraca</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-semibold mb-1.5">CPF do responsável</label>
            <input
              type="text"
              inputMode="numeric"
